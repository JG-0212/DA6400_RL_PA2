...
def main():
    ...
    run.name = repr(reinforce_type1_hyperparameter_list[0]).strip("{}")
    ...
    trainer = Trainer()
    reinforce_type1_results = test_agent(
        env, reinforce_type1_agent, trainer, reinforce_type1_hyperparameter_list, num_experiments=3)

    env.close()

    for score in reinforce_type1_results[0]["means"]:
        wandb.log(
            {
                "score": score
            }
        )
    for moving_avg in reinforce_type1_results[0]["rolling_means"]:
        wandb.log(
            {
    ...
    wandb.log({
        "max_mean_score": np.max(reinforce_type1_results[0]["rolling_means"]),
        "regret": num_episodes*max_return - np.sum(reinforce_type1_results[0]["means"])
    })
