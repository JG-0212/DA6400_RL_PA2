...
def main():
    ...
    for score in ddqn_type2_results["means"]:
        wandb.log(
            {
                "score": score
            }
        )
    for moving_avg in ddqn_type2_results["rolling_means"]:
        wandb.log(
            {
    ...
    wandb.log({
        "max_mean_score": np.max(ddqn_type2_results["rolling_means"]),
        "regret": num_episodes*max_return - np.sum(ddqn_type2_results["means"])
    })
