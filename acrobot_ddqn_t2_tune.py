...
def main():
    ...
    env = RecordVideo(
        env,
        video_folder="backups/acrobot-ddqn-type2-visualizations",
        name_prefix="eval",
    ...
    trainer = Trainer()
    ddqn_type2_results = test_agent(
        env, ddqn_type2_agent, trainer, ddqn_type2_hyperparameter_list, num_experiments=1)

    env.close()

    for score in ddqn_type2_results[0]["means"]:
        wandb.log(
            {
                "score": score
            }
        )
    for moving_avg in ddqn_type2_results[0]["rolling_means"]:
        wandb.log(
            {
    ...
    wandb.log({
        "max_mean_score": np.max(ddqn_type2_results[0]["rolling_means"]),
        "regret": num_episodes*max_return - np.sum(ddqn_type2_results[0]["means"])
    })
